...
/// 设置用户账号
Future<void> signInWithUM(String userID) async {
  if (_supportPlatform()) return;
  _channel.invokeMethod<dynamic>(
...
/// 取消用户账号
Future<void> signOffWithUM() async {
  if (_supportPlatform()) return;
  _channel.invokeMethod<dynamic>('onProfileSignOff');
}

/// 发送自定义事件（目前属性值支持字符、整数、浮点、长整数，暂不支持NULL、布尔、MAP、数组）
Future<void> onEventWithUM(
...
/// 如果需要使用页面统计，则先打开该设置
Future<void> setPageCollectionModeManualWithUM() async {
  if (_supportPlatform()) return;
  _channel.invokeMethod<dynamic>('setPageCollectionModeManual');
}

/// 进入页面统计
Future<void> onPageStartWithUM(String pageName) async {
  if (_supportPlatform()) return;
  _channel.invokeMethod<dynamic>(
...
/// 离开页面统计
Future<void> onPageEndWithUM(String pageName) async {
  if (_supportPlatform()) return;
  _channel.invokeMethod<dynamic>(
...
Future<void> setPageCollectionModeAutoWithUM() async {
  if (_supportPlatform()) return;
  _channel.invokeMethod<dynamic>('setPageCollectionModeAuto');
}

/// 错误发送
Future<void> reportErrorWithUM(String error) async {
  if (_supportPlatform()) return;
  _channel
